...
def glowscript(line, cell):
    ...
    if (len(lst) == 0) or (lst[-1] in ['vpython','rapydscript','coffeescript','javascript']) :
        ...
        if (window.location.protocol === 'https:') {
            ...
            embedScript = "'use strict';import * as acorn from 'https://cdnjs.cloudflare.com/ajax/libs/acorn/3.2.0/acorn.js';require(['https://cdnjs.cloudflare.com/ajax/libs/acorn/3.2.0/acorn.js','https://dl.dropboxusercontent.com/u/5095342/glowscript/lib/jquery/2.1/jquery-ui.custom.min.js','https://dl.dropboxusercontent.com/u/5095342/glowscript/package/glow.2.1.min.js'], function() {" + embedScript + ";$(function(){ window.__context = { glowscript_container: $('#glowscript').removeAttr('id') }; main() });})";
            ...
            var cell_content = """+json.dumps(cell)+""";
            var embedScript = window.glowscript_compile(cell_content, {lang:'"""+lang+"""'}); 
            embedScript = "'use strict';import * as acorn from 'http://cdnjs.cloudflare.com/ajax/libs/acorn/3.2.0/acorn.js';require(['http://cdnjs.cloudflare.com/ajax/libs/acorn/3.2.0/acorn.js','http://www.glowscript.org/lib/jquery/2.1/jquery-ui.custom.min.js','http://www.glowscript.org/package/glow.2.1.min.js'], function() {" + embedScript + ";$(function(){ window.__context = { glowscript_container: $('#glowscript').removeAttr('id') }; main() });})";
            embedScript = embedScript.replace("</", "<\\/"); // escape anything that could be a close script tag... hopefully this sequence only occurs in strings!
            eval(embedScript);
            })
        } else {
